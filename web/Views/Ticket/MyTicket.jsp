    ...
    <div class="my-ticket-container">
        ...
        <div class="search-sort-container">
            <form action="my-ticket" method="get" class="search-sort-form">
                <input type="text" name="search" placeholder="Search by club or area" value="${param.search}"
                       class="search-input">
                <select name="sort" class="sort-select">
                    <option value="price_asc" ${param.sort == 'price_asc' ? 'selected' : ''}>Sort by Price (Low to
                        High)
                    </option>
                    <option value="price_desc" ${param.sort == 'price_desc' ? 'selected' : ''}>Sort by Price (High to
                        Low)
                    </option>
                    <option value="date_asc" ${param.sort == 'date_asc' ? 'selected' : ''}>Sort by Date (Earliest to
                        Latest)
                    </option>
                    <option value="date_desc" ${param.sort == 'date_desc' ? 'selected' : ''}>Sort by Date (Latest to
                        Earliest)
                    </option>
                </select>
                    ...
                    <div class="ticket-details">
                        ...
                        <c:if test="${ticket.status == 'Paid'}">
                            <form action="./my-ticket" method="post">
                                ...
                                <button onclick="return  confirm('Are you sure to refund this ticket?')" type="submit">
                                    Refund
                                </button>
                            </form>
                        </c:if>
