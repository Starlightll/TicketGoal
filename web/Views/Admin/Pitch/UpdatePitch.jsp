    ...
    <form method="POST" action="updatePitchServlet?pitchId=${pitch.pitchId}" enctype="multipart/form-data">
            ...
            <div class="pitch__info">
                <table>
                    <tr class="pitch__name">
                        <td>Name:</td>
                        <td><input type="text" name="pitchName" placeholder="Pitch Name" value="${pitch.pitchName}">
                        </td>
                    </tr>
                ...
                <c:forEach items="${requestScope.AreaList}" var="area">
                    <div class="area">
                        ...
                        <div class="area__management">
                            <a href="?option=update&pitchId=${pitch.pitchId}&areaId=${area.id}" type="button"
                               class="seat__management" id="seat-management-button">Seat</a>
                            <button onclick="deleteArea(${area.id})" type="button" class="delete__button">Delete
                            </button>
                        </div>
                    </div>
                </c:forEach>
        ...
        <div class="area__add__box" id="area-add-box">
            ...
            <div class="box">
                ...
                <p>Name:
                <p><input type="text" name="areaName" id="areaName" placeholder="Enter area name">
                    <button onclick="addArea()" type="button" class="add__button" id="area-add-submit-button">Add
                    </button>
            </div>
        </div>
        <div class="seat__management__box ${param.areaId > 0 ? "show-seat-management-box" : ""}"
             id="seat-management-box">
            <div class="seat__management__header">
                ...
                <div class="seat__management__import">
                    <a style="color: #fff; font-size: 16px;"
                       href="seat?action=import&areaId=${param.areaId}&pitchId=${pitch.pitchId}" class="import__excel"
                       type="submit">Import</a>
                    <button style="font-size: 16px;color: #fff; background: transparent; border: none; outline: none; text-decoration: underline"
                            type="button" class="add__button" id="seat-add-button">Add Seat
                    </button>
                    <a style="font-size: 16px;color: #fff"
                       href="seat?action=deleteAll&areaId=${param.areaId}&pitchId=${pitch.pitchId}" class="delete__all">Delete
                        All</a>
                </div>
            </div>
                ...
                <table>
                    ...
                    <c:forEach items="${seatList}" var="seat">
                        ...
                        <tr>
                            ...
                            <td>
                                <a style="color: #fff"
                                   href="seat?action=edit&pitchId=${pitch.pitchId}&seatId=${seat.seatId}"
                                   class="update__button">Update</a>
                                <a style="color: #fff"
                                   href="seat?action=delete&pitchId=${pitch.pitchId}&seatId=${seat.seatId}&areaId=${seat.areaId}"
                                   class="delete__button">Delete</a>
                            </td>
                        </tr>
                    </c:forEach>
                    </tbody>
